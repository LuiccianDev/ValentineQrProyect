...
const Header: React.FC = () => {
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/50 shadow-md overflow-hidden">
      <div className="relative py-2">
        <div className="flex animate-scroll whitespace-nowrap">
          ...
          {romanticPhrases.map((phrase, index) => (
            <span 
              key={`phrase-1-${index}`}
              className="inline-block px-8 text-sm md:text-base font-semibold text-red-600 italic"
            >
              {phrase}
            </span>
          ))}
